...
export const getTransactionFlow = createSelector(
  [getBudgetItem, getInflowBalance, getOutflowBalance],
  (transaction, inflowBalance, outflowBalance) => {
    const getDescription = () =>
      transaction ? `${transaction.value > 0 ? 'Inflow' : 'Outflow'} Balance (others)` : '';
    const getValue = () =>
      transaction ? Math.abs(transaction.value > 0 ? inflowBalance : outflowBalance) - Math.abs(transaction.value) : 0;
