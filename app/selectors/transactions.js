...
function summarizeTransactions(transactions: Transaction[]): TransactionSummary[] {
	return transactions.reduce((summary, { categoryId, value }) => {
		const sum =
			summary.find((item) => item.categoryId === categoryId) ||
...
export const getTransactionById = (state: State, id: number): Transaction => {
	let transaction = state.transactions.filter((item) => item.id === id);
	return transaction.length === 1 ? transaction[0] : false;
};
...
const getBalance = createSelector([ getTransactions ], (transactions) =>
...
export const getFormattedInflowBalance = createSelector([ getInflowBalance ], (amount) =>
	formatAmount(amount, false)
);

export const getFormattedOutflowBalance = createSelector([ getOutflowBalance ], (amount) =>
	formatAmount(amount, false)
);
